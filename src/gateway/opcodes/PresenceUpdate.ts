...
export async function onPresenceUpdate(this: WebSocket, { d }: Payload) {
    ...
    const presence = ActivitySchema.parse(d);

    await Session.update({ session_id: this.session_id }, { status: presence.status as Status, activities: presence.activities as Activity[] });
