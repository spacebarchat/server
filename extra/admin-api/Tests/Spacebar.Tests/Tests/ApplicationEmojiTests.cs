...
public class ApplicationEmojiTests(ITestOutputHelper testOutputHelper, TestFixture fixture) : TestBed<TestFixture>(testOutputHelper, fixture), IAsyncLifetime {
    ...
    private static ApplicationModel? Application { get; set; }
    ...
    public async ValueTask InitializeAsync() {
        ...
        if (Application is null) {
            var res = await Assert.HttpSuccess(await Client.ApiHttpClient.PostAsJsonAsync("applications", new JsonObject() {
                { "name", "meow" }
            }));
            Application = await res.Content.ReadFromJsonAsync<ApplicationModel>();
        }
    }

    [Fact]
    public async Task GetEmojis() {
        var res = await Assert.HttpSuccess(await Client.ApiHttpClient.GetAsync($"/api/v9/applications/{Application.Id}/emojis", TestContext.Current.CancellationToken));
        var resEmojisObj = await res.Content.ReadFromJsonAsync<JsonObject>();
        var resEmojis = resEmojisObj?["items"]?.AsArray();
    ...
    public async Task UploadEmoji() {
        ...
        var res = await Assert.HttpSuccess(await Client.ApiHttpClient.PostAsJsonAsync($"/api/v9/applications/{Application.Id}/emojis", reqJson, TestContext.Current.CancellationToken));
    ...
    public async Task UpdateEmoji() {
        ...
        var res2 = await Assert.HttpSuccess(await Client.ApiHttpClient.PatchAsJsonAsync($"/api/v9/applications/{Application.Id}/emojis/{resEmoji?["id"]?.GetValue<string>()}", reqJson2,
    ...
    public async Task DeleteEmoji() {
        ...
        await Assert.HttpSuccess(await Client.ApiHttpClient.DeleteAsync($"/api/v9/applications/{Application.Id}/emojis/{resEmoji?["id"]?.GetValue<string>()}",
            TestContext.Current.CancellationToken));
    }
    ...
    public async Task GetEmojiSource() {
        ...
        Assert.Equal("APPLICATION", sourceResJson?["type"]?.GetValue<string>());

        Assert.StringNotNullOrWhitespace(sourceResJson?["application"]?.AsObject()["id"]?.GetValue<string>());
        Assert.Equal(Application.Id.ToString(), sourceResJson?["application"]?.AsObject()["id"]?.GetValue<string>());
    }
}